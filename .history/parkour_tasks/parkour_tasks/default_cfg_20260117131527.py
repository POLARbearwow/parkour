...
class ParkourDefaultSceneCfg(InteractiveSceneCfg):
    ...
    def __post_init__(self):
        ...
        # 如果需要启用，可以在这里覆盖：self.robot.spawn.articulation_props.enabled_self_collisions = True
        
        # 执行器配置已在CustomRobotCfg中完成，使用ImplicitActuatorCfg
        # 如果需要使用ParkourDCMotorCfg，可以在这里覆盖：
        # self.robot.actuators['legs'] = ParkourDCMotorCfg(...)
        
        # 如果项目需要使用ParkourDCMotorCfg而不是ImplicitActuatorCfg，取消下面的注释：
        # self.robot.actuators['legs'] = ParkourDCMotorCfg(
